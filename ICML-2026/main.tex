...
\definecolor{topicshade}{HTML}{F7D1D1}
\definecolor{motiveshade}{HTML}{F7E6B8}
\definecolor{contribshade}{HTML}{FCF9B8}
\definecolor{nuanceshade}{HTML}{D9F4D8}
\definecolor{evidenceshade}{HTML}{D9E6FA}
\definecolor{impactshade}{HTML}{DDF8F8}
\definecolor{boxborder}{HTML}{2B416C}

\newcommand{\theHalgorithm}{\arabic{algorithm}}
\newcommand{\annrow}[3]{%
  \noindent\colorbox{#1}{\parbox[t]{0.17\linewidth}{\scriptsize\bfseries #2}}%
  \hspace{0.012\linewidth}%
  \colorbox{#1!45}{\parbox[t]{0.77\linewidth}{\footnotesize #3}}%
  \par\vspace{0.25em}%
}
\newcommand{\rqbox}[2]{%
  \par\smallskip
  \noindent\fcolorbox{boxborder}{white}{%
    \begin{minipage}{0.96\linewidth}
    \small\textcolor{boxborder}{\textbf{RQ#1:}}~#2
    \end{minipage}}%
  \par\smallskip
}
\newcommand{\takeawaybox}[2]{%
  ...
  \noindent\fcolorbox{boxborder}{white}{%
    \begin{minipage}{0.96\linewidth}
    \small\textcolor{boxborder}{\textbf{RQ#1 Takeaway:}}~#2
    \end{minipage}}%
  \par\smallskip
}
...
% =============================================================================
\begin{abstract}
\annrow{topicshade}{Topic}{LLM safety work often treats the most linearly readable refusal representation as the best place to monitor or intervene.}
\annrow{motiveshade}{Motivation}{This conflates two different questions: where a safety feature is easy to \emph{read}, and where changing it actually \emph{controls} behavior.}
\annrow{contribshade}{Result}{Across 13 instruction-tuned models from 3B--72B, refusal is most readable in the final 2--3\% of layers but most controllable much earlier, producing a 28--59\%-of-depth gap.}
\annrow{nuanceshade}{Mechanism}{Ctrl-peak ablation changes behavior while leaving the late refusal projection nearly intact on 11/12 models, so the late ``refusal direction'' is an output-adjacent readout, not the causal variable.}
\annrow{evidenceshade}{Evidence}{Dual-level attack sweeps, per-layer separation/probe measurements, logit-lens probes, path patching, random-direction controls, and held-out-prompt validation all support the gap.}
\annrow{impactshade}{Takeaway}{Safety systems should \textbf{intervene at the controllability peak} but \textbf{detect at the separability peak}; the attack surface is trainable away, but current LoRA counterexample training over-refuses safe XSTest prompts.}
\end{abstract}
...
\rqbox{1}{Where does refusal become most readable, and where is it most controllable?}
\rqbox{2}{Does this readability--controllability gap persist across model families, scales, and reasoning-trained variants?}
\rqbox{3}{Why can a mid-layer intervention change behavior while the late refusal readout remains intact?}
\rqbox{4}{What design rule follows for attacks, monitors, and trainable defenses?}
...
\takeawaybox{1--2}{Readability peaks late, controllability peaks mid-network, and the separation is not a single-model artifact: it appears across all 13 models with a 28--59\%-of-depth gap.}
...
Appendix Figure~\ref{fig:decoupling} visualizes the probes at the separability peak; raw numbers are in App.~\ref{app:mech_raw}.

\paragraph{Ablation does not move the readout.}
On 11 of 12 models, ctrl-peak ablation changes behavior while leaving the late refusal projection nearly unchanged; on five Qwen models the projection even increases. Sockpuppeting, in contrast, flips the late readout on most models. Thus the refusal direction at the separability peak behaves like an output-adjacent readout, not the causal variable that controls refusal. Full interpretation and model-level exceptions are in App.~\ref{app:moved_details}.

\takeawaybox{3}{The late refusal direction is a readout of behavior, not the causal knob: behavior can change before the final-layer refusal projection moves.}
...
The dual attack reaches 54--88\% ASR on held-out prompts on 10/13 models (App.~\ref{app:disjoint}); the refusal direction beats random unit-vector controls by 3.9--32.7$\times$ (App.~\ref{app:randdir}); and ablation does not corrupt benign generation (App.~\ref{app:utility}). A sep-peak compliance probe transfers across dual, sockpuppet, and GCG-style attacks, while LoRA counterexample training removes the ablation surface but introduces XSTest over-refusal (App.~\ref{app:defense}, \ref{app:advtrain}). We therefore view the gap as both a vulnerability and a trainable diagnostic surface.

\takeawaybox{4}{Use different layers for different safety functions: intervene at the ctrl peak, monitor at the sep peak, and train against ctrl-peak counterexamples with an explicit benign-preservation loss.}

\paragraph{Reasoning-model failure modes.}
QwQ-32B and DS-R1-Distill-Llama-70B leak harmful content inside their \texttt{<think>} block before the final refusal, an axis orthogonal to the readability--controllability gap (App.~\ref{app:cotleak}). Output-level safety is therefore incomplete for reasoning models.
...
\section*{Limitations}
\textbf{Coverage:} Llama-3.3-70B and Mistral-Large-123B were inaccessible; Gemma-4-31B was swept but unjudged. \textbf{Judges:} Opus 4.6 + Sonnet 4.6 agree above 94\%; human evaluation is future work. \textbf{Utility:} LoRA counterexample training removes ablation-ASR but over-refuses XSTest-safe, so benign-preservation remains open.
...
\textbf{M} & Additional Main-Text Details \dotfill & \pageref{app:moved_details} \\
\textbf{N} & Additional Generated Concept Figures \dotfill & \pageref{app:generated_figures} \\
\end{tabular*}
\vspace{1.5em}
...
\clearpage
\section{Additional Main-Text Details}
\label{app:moved_details}

\paragraph{Mechanistic interpretation.}
On 11 of 12 models the ablation column differs from the no-attack column by less than 10\% on $p_{\text{sep}}$ and less than 0.6 on $\Delta$logit. On five Qwen models the projection even \emph{increases} under ablation (the signal rebuilds and overshoots). Gemma-3-27B is the only model showing a meaningful projection reduction (30\%), still smaller than the effect of sockpuppeting. Yet the behavioral ASR under ablation-only ranges from 11.5\% (Qwen2.5-32B) to 89.5\% (Qwen2.5-3B)---a 78-point spread driven by an intervention that does not change either the refusal projection at the readability peak or the first-token logit preference. Sockpuppeting, in contrast, flips both on most models.

The combined evidence reframes the gap. The ``refusal direction'' measured at the separability peak captures an output-adjacent readout: the residual stream at the last layer is shaped so that the unembedding matrix projects refusal-token logits high. Ablation at the controllability peak changes behavior through orthogonal circuit modifications (likely attention patterns and OV-circuit writes that do not lie on the linear refusal axis) but the residual stream is rebuilt by subsequent layers into a representation that, when unembedded, still reads as refusal. This explains why the late-layer refusal signal rises after mid-layer ablation on Qwen models, and why intervening at the readability peak is strictly worse for behavioral effects.

\paragraph{Defense and training details.}
A linear compliance-prediction probe achieves best-AUC at the sep peak on 8/9 models with ctrl-peak AUC 1--27 pts lower (App.~\ref{app:defense}), and the same probe transfers to GCG~\citep{zou2023universal} with 39--104-unit margins (App.~\ref{app:gcg}). Compared with Llama Guard 3, which is bypassed by assistant prefill, and Circuit Breakers~\citep{zou2024circuit}, which produces $\sim$83--90\% incoherent outputs, the sep-peak probe is the cheapest cross-attack monitor (App.~\ref{app:defbase}). Path patching shows the rebuild is distributed: recovery climbs smoothly across three families with no localized relay (App.~\ref{app:pathpatch}). LoRA fine-tuning (200 steps, $r$=16) with ablation counterexamples drives ablation-ASR to 0\% on held-out prompts across five models (2000-response dual-judge re-evaluation, App.~\ref{app:advtrain}); however, those same adapters over-refuse XSTest-safe at 85--99\%~\citep{rottger2024xstest}, so the recipe needs a benign-preservation component before deployment.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{figures/generated_pdf/fig_mechanistic_decoupling_banana_upscaled.pdf}
